...
def test(
    ...
    mse = np.zeros(n)
    for i in range(n):
        acc[i], mse[i] = tpr_fixed_key(
            net, sample_size, keys[i], nr=nr, diff=diff, batch_size=batch_size
        )
    worst = np.argmax(mse)
    best = np.argmin(mse)
    # determine mse and tpr again for worst and best index
    mse_worst, acc_worst = tpr_fixed_key(
        net, test_size, keys[worst], nr=nr, diff=diff, batch_size=batch_size
    )
    mse_best, acc_best = tpr_fixed_key(
        net, test_size, keys[best], nr=nr, diff=diff, batch_size=batch_size
    )
