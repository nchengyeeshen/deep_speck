...
s8 = np.load("data_wrong_key_8r_std_1e6.npy")
s8 = 1.0 / s8
m7 = np.load("data_wrong_key_mean_7r.npy")
s7 = np.load("data_wrong_key_std_7r.npy")
s7 = 1.0 / s7
m6 = np.load("data_wrong_key_mean_6r.npy")
s6 = np.load("data_wrong_key_std_6r.npy")
s6 = 1.0 / s6
...
def bayesian_key_recovery(
    cts, net=net7, m=m7, s=s7, num_cand=32, num_iter=5, seed=None
):
    n = len(cts[0])
    ...
    if not seed is None:
        keys = np.copy(seed)
    ct0a, ct1a, ct0b, ct1b = (
        np.tile(cts[0], num_cand),
        np.tile(cts[1], num_cand),
        np.tile(cts[2], num_cand),
        np.tile(cts[3], num_cand),
    )
    ...
    for i in range(num_iter):
        k = np.repeat(keys, n)
        c0a, c1a = sp.decrypt_one_round((ct0a, ct1a), k)
        c0b, c1b = sp.decrypt_one_round((ct0b, ct1b), k)
        X = sp.convert_to_binary([c0a, c1a, c0b, c1b])
        Z = net.predict(X, batch_size=10000)
...
def test_bayes(
    ...
    while improvement:
        k1, k2, val = verifier_search(
            [cts[0][best_pod], cts[1][best_pod], cts[2][best_pod], cts[3][best_pod]],
            best_key,
            net=net_help,
            use_n=verify_breadth,
        )
...
def test(
    ...
    for i in range(n):
        ...
        ct, key = gen_challenge(
            num_structures, nr, neutral_bits=neutral_bits, keyschedule=keyschedule
        )
        g = find_good(ct, key)
        g = np.max(g)
        good[i] = g
        guess, num_used = test_bayes(
            ct,
            it=it,
            cutoff1=cutoff1,
            cutoff2=cutoff2,
