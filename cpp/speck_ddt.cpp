...
void make_examples(uint32_t nr, uint32_t diff, vector<uint32_t> &v0,
                   vector<uint32_t> &v1, vector<uint32_t> &w) {
  random_device rd;
...
#pragma omp parallel for
  for (uint64_t i = 1; i < small; i++) {
    ...
    double p = 0;
    uint32_t ind = in1 << WORD_SIZE;
    if (sums[in1] != 0)
      for (uint32_t in2 = 0; in2 <= MASK_VAL; in2++) {
        uint32_t index = ind ^ in2;
        uint32_t inp = (in2 << WORD_SIZE) ^ in1;
...
void calc_ddt(uint32_t in_diff, int num_rounds) {
  ...
  for (int i = 0; i < num_rounds; i++) {
    ...
    cout << "Rounds: " << dec << (i + 1) << ", Acc: " << acc << ", tpr:" << tpr
         << ", tnr:" << tnr << endl;
    stringstream del;
    del << hex << in_diff;
